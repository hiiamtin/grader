...
class Supervisor extends MY_Controller {
	...
	public function edit_profile_action() {
		...
		if (strlen($new_password) >= 4 && $new_password==$confirm_password)		
		{
			...
			$this->load->model('supervisor_model');
			$this->supervisor_model->update_supervisor_password(md5($new_password));
	...
	public function group_management() {
		...
		$this->load->model('lab_model');
		$supervised_groups = $this->lab_model->get_supervise_group($user_id); // return array of group_id
	...
	private function get_data_for_testcase($stu_id,$exercise_id) {
		...
		$data_for_testcase['num_of_testcase'] = $number_of_testcase;
	...
	public function chapter_dashboard($group,$chapter) {
		//check whether this supervisor has access right
		...
		$this->load->view('supervisor/nav_fixtop');
		$this->load->view('supervisor/nav_sideleft');
		$this->load->view('supervisor/chapter_dashboard',$data);
		$this->load->view('supervisor/footer');
	}
	...
	* * */
	...
	public function exam_room_seating_chart_dashboard($roomNum) {
		/// หน้าแสดงแผนผังที่นั่งห้องสอบ
		/// Url จะเป็น exam_room_seating_chart/{เลขห้อง}
		$this->load->model('lab_model');
		$this->load->model('supervisor_model');
